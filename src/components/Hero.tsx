  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-bdi-cream p-5 pointer-events-none select-none">
        <h1 id={titleId} className="font-ranchers text-[clamp(3rem,8vw,7rem)] leading-[1.1] mb-4">
          {words.map((w, i) => (
            <span key={i} className="word inline-block mr-2">{w}</span>
          ))}
        </h1>
        <p className="font-ranchers text-[clamp(1.3rem,3vw,2.2rem)] opacity-90 mb-7">
          {t('heroSub')}
        </p>
      ...
      <div className="hero-click-hint absolute bottom-[70px] left-1/2 -translate-x-1/2 z-10 font-ranchers text-base text-[rgba(255,248,240,0.6)] animate-[fadeInOut_3s_ease_infinite]">
        {t('heroClickHint')}
      </div>
    </section>
  );
